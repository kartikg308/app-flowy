...
class _DesktopEditorState extends State<DesktopEditor> {
      ...
      items: [
        ...
        ToolbarItem(
          ...
          builder: (context, editorState, c1, _) {
            ...
            print("nodes in selection $nodes");
            print("current cursor position ${editorState.selection?.toJson()}");
            final isHighlight = nodes.allSatisfyInSelection(selection, (delta) {
              return delta.everyAttributes(
                (attributes) => attributes["formula"] == true,
              );
            });
            return IconButton(
              onPressed: () async {
                ...
                final afterSelection = Selection(
                  start: Position(
                    path: node!.path,
                    offset: selection.end.offset + selection.length,
                  ),
                  end: Position(
                    path: node.path,
                    offset: selection.end.offset + selection.length,
                ...
                print("start index is $startIndex, length is $length");
  ...
  String? _buildAutoCompleteTextProvider(
    ...
    final selection = editorState.selection;
    final delta = node.delta;
    if (selection == null || delta == null || !selection.isCollapsed || selection.endIndex != delta.length || !node.path.equals(selection.start.path)) {
      return null;
    }
