...
class _AppFlowyRichTextState extends State<AppFlowyRichText> with SelectableMixin {
  ...
  TextSpanDecoratorForAttribute? get textSpanDecoratorForAttribute => widget.textSpanDecoratorForCustomAttributes ?? widget.editorState.editorStyle.textSpanDecorator;

  AppFlowyAutoCompleteTextProvider? get autoCompleteTextProvider => widget.autoCompleteTextProvider ?? widget.editorState.autoCompleteTextProvider;
    ...
    for (final textInsert in textInserts) {
      ...
      if (attributes?["formula"] ?? false) {
        ...
        textSpan = TextSpan(
          children: [
            widgetSpan,
            ...List.generate(textInsert.length - 1, (i) => const WidgetSpan(child: SizedBox())),
          ],
        );
      }
      textSpans.add(
      ...
      print("new offset is ${textInsert.length}");
    }
    return TextSpan(
      children: textSpans,
    );
